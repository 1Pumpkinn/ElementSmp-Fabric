...
public class ItemEventListeners {
    ...
    private static boolean handleUpgrader(ServerPlayerEntity player, ItemStack stack, ServerWorld world) {
        ...
        if (currentElement == null) {
            player.sendMessage(Text.literal("You need an element before you can upgrade!").formatted(net.minecraft.util.Formatting.RED), false);
            return false;
        }
        ...
        if (currentLevel >= upgraderLevel) {
            player.sendMessage(Text.literal("You already have this upgrade level or higher!").formatted(net.minecraft.util.Formatting.YELLOW), false);
            return false;
        }
        ...
        world.playSound(null, player.getBlockPos(), SoundEvents.UI_TOAST_CHALLENGE_COMPLETE, SoundCategory.PLAYERS, 1.0f, 1.2f);

        player.sendMessage(Text.literal("✦ Upgraded to Level " + (upgraderLevel == 1 ? "I" : "II") + "! ✦")
                .formatted(net.minecraft.util.Formatting.GREEN, net.minecraft.util.Formatting.BOLD), false);
    ...
    private static boolean handleReroller(ServerPlayerEntity player, ItemStack stack, ServerWorld world) {
        ...
        for (ElementType type : ElementType.values()) {
            if (type != ElementType.LIFE && type != ElementType.DEATH) basicElements.add(type);
        }

        if (basicElements.isEmpty()) {
            player.sendMessage(Text.literal("No elements available to reroll!").formatted(net.minecraft.util.Formatting.RED), false);
            return false;
        }
        ...
        player.sendMessage(Text.literal("✦ Your element has been rerolled to ")
                .formatted(net.minecraft.util.Formatting.GOLD)
                .append(Text.literal(newElement.name()).formatted(net.minecraft.util.Formatting.AQUA))
                .append(Text.literal("! ✦").formatted(net.minecraft.util.Formatting.GOLD)), false);
    ...
    private static boolean handleAdvancedReroller(ServerPlayerEntity player, ItemStack stack, ServerWorld world) {
        ...
        for (ElementType type : ElementType.values()) {
            if ((type == ElementType.LIFE && pd.hasElementItem(ElementType.LIFE)) ||
                    (type == ElementType.DEATH && pd.hasElementItem(ElementType.DEATH)) ||
                    (type != ElementType.LIFE && type != ElementType.DEATH)) {
        ...
        world.playSound(null, player.getBlockPos(), SoundEvents.BLOCK_ENCHANTMENT_TABLE_USE, SoundCategory.PLAYERS, 1.0f, 0.8f);

        player.sendMessage(Text.literal("✦ Your element has been rerolled to ")
                .formatted(net.minecraft.util.Formatting.DARK_PURPLE)
                .append(Text.literal(newElement.name()).formatted(net.minecraft.util.Formatting.LIGHT_PURPLE))
                .append(Text.literal("! ✦").formatted(net.minecraft.util.Formatting.DARK_PURPLE)), false);
    ...
    private static boolean handleCoreConsumption(ServerPlayerEntity player, ItemStack stack, ServerWorld world) {
        ...
        if (pd.hasElementItem(type)) {
            player.sendMessage(Text.literal("You have already consumed the " + type.name() + " Element!").formatted(net.minecraft.util.Formatting.RED), false);
            return false;
        }
        ...
        if ((type == ElementType.LIFE && mod.getDataStore().isLifeElementCrafted()) ||
            ...
            player.sendMessage(Text.literal(type.name() + " Element has already been crafted!").formatted(net.minecraft.util.Formatting.RED), false);
            return false;
        }
        ...
        player.sendMessage(Text.literal(emoji + " You consumed the ")
                .formatted(type == ElementType.LIFE ? net.minecraft.util.Formatting.GREEN : net.minecraft.util.Formatting.DARK_GRAY)
                .append(Text.literal(type.name() + " Element").formatted(net.minecraft.util.Formatting.BOLD))
                .append(Text.literal("! " + emoji).formatted(type == ElementType.LIFE ? net.minecraft.util.Formatting.GREEN : net.minecraft.util.Formatting.DARK_GRAY)), false);
