...
public class ElementCommand {
    ...
    private static int setElement(CommandContext<ServerCommandSource> context) {
        try {
    ...
    private static int debugElement(CommandContext<ServerCommandSource> context) {
        try {
            ServerPlayerEntity target = EntityArgumentType.getPlayer(context, "player");
            ElementType element = ElementMod.getInstance().getElementManager()
                    .getPlayerElement(target);
